...
namespace AStar
{
    ...
    public:
        ...
        Generator() {
            ...
            */
            ...
            centerline_contributing_factor_sq = 0.8* 0.8;
        }
        ...
        // Set the centerline points and build KDTree
        ...
        // Set contributing factor
        void setCenterlineContributingFactor(double factor){
            this->centerline_contributing_factor = factor;
        }
        void setCenterlineContributingFactorSq(double factor){
            this->centerline_contributing_factor_sq = factor;
        }

        // Get squared distance to nearest centerline point
        double distance_to_centerline_sq(const Vec2i& coord) const {
                                    ...
                                    const Vec2i& targetPos, double targetTheta)
        {
            ...
            while (!openQueue.empty() && iter < MAX_ITER) {
                ...
                for (double dTheta : headingChanges){
                    ...
                    // --- LIMIT TOTAL TURN ---
                    /*
                    The reason this is 80 degrees is explained below:
                        Car's minimum turning radius is 5,5 meters. 
                        With a 5,5 turning radius, car will be able to turn at most 58,3 degrees in the worst case scenario for turning.
                        Ask Umut to learn for more if this explanation wasn't understandable.
                    */
                    double deltaFromStart = normalize_angle(newTheta - sourceTheta);
                    if (std::fabs(deltaFromStart) > 80.0) // max ±80°, adjust as needed
                        continue;
                    ...
                    double step_length = 0.85 * curvature_contributing_factor;
                    Pose nxt = simulate_arc(currentPose, dTheta, step_length, 5.20); // !!5.50 is actual turning.!! min turning radius from center of gravity
                    ...
                    double stepCost = 10;
                    double turnCost = static_cast<uint>(std::fabs(dTheta));
                    if (std::fabs(dTheta) > 1e-9){
                        stepCost += turnCost;
                        stepCost += 5;
                    }
                    double stepCostSq = stepCost * stepCost;
                    double dist_sq = distance_to_centerline_sq(newCoord);
                    double newG = current->G + stepCostSq + centerline_contributing_factor_sq * dist_sq;
                    ...
                    //LAZY EXPANSION
                    double newScore = newG + heuristicFunc(newCoord, newTheta, targetPos, targetTheta);
                    double oldScore = it != openMap.end() ? it->second->getScore() : std::numeric_limits<double>::max();
    ...
    private:
        ...
        double centerline_contributing_factor_sq;
