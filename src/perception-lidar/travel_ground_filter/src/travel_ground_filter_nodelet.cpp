...
namespace ground_segmentation {
  ...
  TravelGroundFilterComponent::TravelGroundFilterComponent(const rclcpp::NodeOptions& options) 
      ...
      INPUT_TOPIC_ = this->declare_parameter<std::string>("input_topic", "nonground");
  ...
  void TravelGroundFilterComponent::point_cloud_callback(
    ...
    RCLCPP_INFO(this->get_logger(), "Input cloud size: %u", point_cloud->width * point_cloud->height);
    ...
    RCLCPP_INFO(this->get_logger(), "Total time: %.2f ms", std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(end-start).count());
    ...
    nonground_cloud_msg.header.frame_id = FRAME_ID_;
