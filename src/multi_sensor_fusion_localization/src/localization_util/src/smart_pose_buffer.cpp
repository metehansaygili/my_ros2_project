...

  {
    ...
    if (pose_buffer_.size() < 2) {
      /*
        RCLCPP_INFO(
            logger_,
            "pose_buffer_.size() < 2. Current size: %zu",
            pose_buffer_.size()
        );*/
        return std::nullopt;
    }
...
/*
  if (target_ros_time < time_first) {
      RCLCPP_INFO(
          logger_,
          "Mismatch between pose timestamp and current timestamp. Target time: %ld, First time: %ld",
          target_ros_time.nanoseconds(),
          time_first.nanoseconds()
      );
      return std::nullopt;
  }
*/
  ...
  RCLCPP_INFO(logger_, "interpolated");
