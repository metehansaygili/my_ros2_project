    ...
    GetDifferentialPointCloudMap::Response::SharedPtr &response) const
{
  ...
  for (const auto &ele : all_pcd_file_metadata_dict_)
  {
    ...
    if (!isGridWithinQueriedArea(area, metadata))
      continue;
...
{
  RCLCPP_ERROR_STREAM(logger_, "TEST");
  auto area = req->area;
  RCLCPP_ERROR_STREAM(logger_, area.center_x << " " << area.center_y << " " << area.radius);

  std::vector<std::string> cached_ids = req->cached_ids;
  differentialAreaLoad(area, cached_ids, res);
  for (auto id : res->new_pointcloud_with_ids)
  {
    RCLCPP_ERROR_STREAM(logger_, id.cell_id);
  }
